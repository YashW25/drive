...
export class SessionService implements OnModuleDestroy, OnModuleInit, OnApplicationBootstrap {
  ...
  async create(dto: CreateSessionDto, ownerSupabaseId?: string): Promise<Session> {
    ...
    if (ownerSupabaseId) {
      ...
      if (existingForUser) {
        throw new ConflictException(`User already has a session`);
      }
    }
  ...
  async findAll(allowedSessions?: string[] | null, ownerSupabaseId?: string): Promise<Session[]> {
