...
import { RequireRole, CurrentApiKey, SessionScoped, CurrentUser } from '../auth/decorators/auth.decorators';
...
export class SessionController {
  ...
  async create(@Body() dto: CreateSessionDto, @CurrentUser() user?: any): Promise<Session> {
    const session = await this.sessionService.create(dto, user?.supabaseId);
  ...
  async findAll(@CurrentApiKey() apiKey?: ApiKey, @CurrentUser() user?: any): Promise<SessionResponseDto[]> {
    ...
    const sessions = await this.sessionService.findAll(apiKey?.allowedSessions, user?.supabaseId);
