...
const BAILEYS_BROWSER: [string, string, string] = ['Ubuntu', 'Chrome', '20.0.0'];
...
export class BaileysAdapter implements IWhatsAppEngine {
  ...
  private readonly authStorageProvider: import('../storage/auth-storage.interface').IAuthStorageProvider;
  ...
  constructor(private readonly config: BaileysAdapterConfig) {
    this.authStorageProvider = config.authStorageProvider ?? (() => {
      const { SupabasePgAuthStorageProvider } = require('../storage/supabase-pg-auth-storage.provider');
      return new SupabasePgAuthStorageProvider();
    })();
  ...
  private async connectInner(): Promise<void> {
    ...
    const { state, saveCreds } = await this.authStorageProvider.useAuthState(this.config.sessionId);
    ...
    const sock = b.default({
      ...
      // Enable the initial sync conditionally. Returning false disables ALL history + app-state sync
      // (no contacts, chats, recent history). This drastically reduces RAM usage.
  ...
  private async clearAuthState(): Promise<void> {
    try {
      await this.authStorageProvider.clearAuthState(this.config.sessionId);
      this.logger.log('Cleared Baileys auth state', { sessionId: this.config.sessionId });
    } catch (err) {
