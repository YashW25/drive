...
OpenWA includes a built-in, ultra-lightweight heartbeat endpoint designed for external uptime monitors (like cron-job.org or UptimeRobot). It runs in under 10ms, consumes virtually no CPU or memory, and avoids touching the WhatsApp connection or processing messages.
...
```
*(If WhatsApp is disconnected, `status` will return `"degraded"`)*

### Environment Variables
| Variable | Default | Description |
|---|---|---|
| `HEARTBEAT_ENABLED` | `true` | Enable or disable the heartbeat service. |
| `HEARTBEAT_LOGGING` | `false` | Log every heartbeat ping to the console. |
| `HEARTBEAT_ROUTE` | `/api/heartbeat` | The custom route path for the heartbeat endpoint. |
| `HEARTBEAT_SHOW_MEMORY` | `true` | Include `memoryMB` in the response. |
| `HEARTBEAT_SHOW_UPTIME` | `true` | Include `uptimeSeconds` in the response. |
| `HEARTBEAT_SHOW_VERSION` | `true` | Include `version` in the response. |

### Recommended External Monitoring Setup
**Service**: cron-job.org  
**Method**: GET  
**URL**: `https://YOUR_RENDER_APP.onrender.com/api/heartbeat`  
