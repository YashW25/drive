...
class WhatsAppWebJsAdapter extends events_1.EventEmitter {
    ...
    async getChannelById(channelId) {
        this.ensureReady();
        try {
            const ch = await this.client.getChannelById(channelId);
            if (!ch) {
                return null;
            }
            return {
    ...
    async postTextStatus(_text, _options) {
        this.ensureReady();
        throw new engine_not_supported_error_1.EngineNotSupportedError('postTextStatus');
    }
    async postImageStatus(_media, _caption) {
        this.ensureReady();
        throw new engine_not_supported_error_1.EngineNotSupportedError('postImageStatus');
    }
    async postVideoStatus(_media, _caption) {
        this.ensureReady();
        throw new engine_not_supported_error_1.EngineNotSupportedError('postVideoStatus');
    }
