...
class BaileysAdapter {
    ...
    async mapMessage(msg, contentType) {
        const b = await this.loadLib();
    ...
    captureHistoryMessages(messages) {
        if (!messages.length) {
            return;
        }
        const nameUpdates = [];
