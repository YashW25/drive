...
let PluginLoaderService = class PluginLoaderService {
    ...
    pluginsDir;
    constructor(configService, hookManager, pluginStorage, moduleRef) {
    ...
    resolveEngine(manifest, sessionId) {
    ...
    resolveEngineRead(manifest, sessionId) {
        this.assertPermission(manifest, plugin_interfaces_1.PluginCapabilityPermission.ENGINE_READ);
        return this.resolveEngine(manifest, sessionId);
    }
    createSandboxHost(capDispatcher, onHookSubscribe, onLog) {
        ...
        }), capDispatcher, onHookSubscribe, onLog);
    }
    ...
    async enableSandboxed(pluginId, plugin) {
        ...
        const host = this.createSandboxHost((verb, args) => (0, capability_router_1.dispatchCapabilityVerb)(context, verb, args), onHookSubscribe, onLog);
    ...
    createPluginContext(plugin) {
        ...
        return {
            ...
            engine: {
                getGroupInfo: async (sessionId, groupId) => this.resolveEngineRead(plugin.manifest, sessionId).getGroupInfo(groupId),
                getContacts: async (sessionId) => this.resolveEngineRead(plugin.manifest, sessionId).getContacts(),
                getContactById: async (sessionId, contactId) => this.resolveEngineRead(plugin.manifest, sessionId).getContactById(contactId),
                checkNumberExists: async (sessionId, phone) => this.resolveEngineRead(plugin.manifest, sessionId).checkNumberExists(phone),
            ...
            net: {
                fetch: async (url, init) => {
                    ...
                    if (!(0, plugin_net_1.isNetHostAllowed)(plugin.manifest.net?.allow, url)) {
                        throw new plugin_interfaces_1.PluginCapabilityError(`Plugin ${plugin.manifest.id} may not fetch ${url} — add its host to the manifest net.allow list`);
                    }
