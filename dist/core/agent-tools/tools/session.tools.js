...
function sessionTools(session) {
    return [
        {
            name: 'SessionFindAll',
            description: 'List the WhatsApp sessions this API key may access (id, name, status). Use to discover available sessions before calling session-scoped tools.',
            tier: 'read',
            inputSchema: zod_1.z.object({}),
            handler: (_input, apiKey) => session.findAll(apiKey.allowedSessions).then(ss => ss.map(s => session_response_dto_1.SessionResponseDto.fromEntity(s))),
        },
        {
            name: 'SessionFindOne',
